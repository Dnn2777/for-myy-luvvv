...
screen = trtl.Screen()
screen.bgcolor("#F8F8FF")  # Changed to an even lighter purple (ghost white with purple tint)
screen.setup(900, 800)
screen.title("Happy Birthday My Luvv")
...
screen.delay(0)
color_palette = {
    "light_pink": "#FFB6C1",  # Added light pink
    "medium_pink": "#FF69B4",  # Added medium pink
    "white": "#FFFFFF",
    "light_yellow": "#f7e8aa",
    "cream": "#fffceb",
    "off_white": "#fffdf4",
    ...
    "sky_blue": "#87ceeb",
    "light_steel": "#b0c4de",
    "purple": "#800080",  # Added purple for flowers
...
def draw_flower(t, x, y, size, color):
    # Draw stem first (so it's behind the flower)
    t.penup()
    t.goto(x, y - size - 50)  # Start below the flower
    t.pendown()
    t.pencolor("green")  # Stem color
    t.pensize(3)
    t.goto(x, y)  # Draw stem up to flower base
    t.penup()
    
    # Now draw the flower petals
    t.goto(x, y)
    t.pendown()
    t.pencolor(color)
    for _ in range(6):
    ...
    t.goto(x, y + size)
    t.pendown()
    t.dot(size/2, "yellow")  # Center of flower

# Draw many small and medium hearts in the background (behind everything)
for _ in range(200):  # Increased to even more hearts
    size = random.choice([8, 12, 16, 20, 24, 28])  # Small to medium sizes
    x = random.randint(-450, 450)
    y = random.randint(-400, 400)
    draw_heart(pen, x, y, size, random.choice(["#FF69B4", "#FFB6C1", "#FFC0CB", "#FF1493", "#FF6347"]))  # Various pink shades

# Draw many purple flowers at the bottom, evenly spaced from left to right, with larger varying sizes and stems
flower_y = -350
flower_spacing = 60  # Closer spacing for more flowers
for x in range(-450, 451, flower_spacing):
    size = random.choice([25, 30, 35])  # Larger sizes than before
    draw_flower(pen, x, flower_y, size, color_palette["purple"])

# Draw cake (cake colors changed to pink and white from top to bottom)

draw_cake_layer(pen, 150, 60, "white", color_palette["light_pink"], 0)
...
for deg in range(180, 360):
    ...
    y = ellipse_y(60, deg)
    pen.goto(x, y)
pen.fillcolor(color_palette["medium_pink"])
pen.end_fill()

draw_cake_layer(pen, 120, 48, "white", color_palette["white"], 0)
...
for deg in range(540):
    ...
    pen.goto(x, y)
pen.goto(-120, 0)
pen.fillcolor(color_palette["white"])
pen.end_fill()

draw_cake_layer(pen, 120, 48, color_palette["white"], color_palette["white"], 70)
...
pen.forward(230)
pen.pendown()
pen.color("purple")  # Set font color to purple
pen.write("Happy Birthday", font=("Bevan Script MT", 50, 'bold'))

pen.penup()
pen.right(30)
pen.forward(-85)
pen.write(" Raden Ayu Gisel ", font=("Brush Script MT", 40, 'bold'))
